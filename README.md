...
- ただし**右クリックメニューは Windows と macOS で実機検証済み、Linux は未検証**です（Linux での検証は上記より後に追加した機能のため未実施）。Linux では、メニュー項目の一部が使えない・グレーアウトの判定が実態と合わない可能性があります。同等の操作はキーボードショートカットとメニューバーの「編集」から行えます。
