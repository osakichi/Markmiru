...
- **明示的な改ページとダーク系スタイルの印刷配色（2026-09-25 の追加）は Windows・macOS・Linux の 3 OS すべてで実機検証済み**（Linux は 2026-09-26。Ubuntu 24.04・WebKitGTK 4.1・正式ビルド `1b8e1b3`。macOS は 2026-09-26。macOS 26.7・Apple Silicon・正式ビルド `e6878f1`）。macOS の検証中に見つかった 2 件（ダーク系スタイルの印刷でコードの太字・斜体が残る・遮断した外部画像の文言が WebKit で表示されない）はいずれも今回の追加以前からの挙動で、2026-09-26 に対応済み（コードの印刷用テーマがダーク側の太字・斜体・淡い文字色を打ち消す／遮断画像を文言付きの `span` に置き換える。3 OS の実アプリで確認済み〔macOS は正式ビルド `8943d36`・Linux は正式ビルド `152cedf`。いずれも 2026-09-26〕。`docs/アーキテクチャ・画面設計.md` §10「既知の制約・課題」に記録）。詳細は `docs/アーキテクチャ・画面設計.md` §5.5「PDF 出力 / 印刷」および `docs/スタイル設定設計.md` §5.2 を参照
