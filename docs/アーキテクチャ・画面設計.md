...
| 編集 | **macOS**: Wails `EditMenu()`（ネイティブ標準メニュー。元に戻す/コピー/貼り付け 等）。**Windows・Linux**: 日本語ラベルで手組み（取り消し/やり直し/切り取り/コピー/貼り付け/すべて選択）。各項目は `menu:*` を発火し、右クリックメニューと共有の `editActions.ts` で実行（クリップボードは Go ランタイム経由） |
...
> **編集メニューの実装が macOS と Windows/Linux で分かれる理由**: Wails のロールメニュー（`EditMenu()` 等）は darwin ネイティブ層でのみ展開され、Windows/Linux では無視されて空メニューになる。そのため非 mac では手組みする。手組み側は Ctrl+C/V/X/A/Z をアクセラレータ登録しない（WebView/CodeMirror のネイティブなキー処理＝フォーカス文脈依存の挙動を奪わないため。メニューはクリック呼び出し用）。
