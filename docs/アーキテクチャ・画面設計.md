...
| `SetEditMenuEnabled(canEdit)` | 手組み編集メニュー（Win/Linux）の編集専用項目（取り消し/やり直し/切り取り/貼り付け）の有効・無効をモードに応じて切替（§9） |
| `FocusWindow()` / `Quit()` | WebView にキーボードフォーカスを付与（アプリ内ダイアログ表示時。Windows 用、他 OS は no-op） / アプリ終了。※ウィンドウの前面化は非公開の `bringToFront()`（IPC 受信時）が担当 |

Go→フロントのイベント: `ipc:open-file`（後発インスタンスから転送されたファイルを開く。§5.7）、`menu:<action>`（メニュー操作。例: `menu:new` `menu:open` `menu:save` `menu:saveAs` `menu:print` `menu:style-import` `menu:style-export` `menu:toggleMode` `menu:toggleSidebar` `menu:license` `menu:about`。手組み編集メニュー（Win/Linux）からは `menu:undo` `menu:redo` `menu:cut` `menu:copy` `menu:paste` `menu:selectAll` `menu:find` も。§9）、`app:request-quit`（終了要求 → フロントが未保存確認の上で `Quit()`）。
...
- **状態管理**: Svelte stores で `tabs`（`activeTabId` 含む）/ `ui`（サイドバー開閉等）/ `style`（アクティブスタイル）を保持。
...
├─ ConfirmDialog.svelte  … 確認ダイアログ（終了時の未保存確認・インポート上書き確認 等。DialogShell を内包）
├─ ExportPickerDialog.svelte … エクスポート対象スタイルの選択（プリセットはグレー表示）
├─ RiskyConfirmDialog.svelte … 危険な選択の確認（外部リンク/外部画像。危険側はポインタのみ・起動遅延。§5.11）
└─ ContextMenu.svelte    … 両モードの右クリックメニュー（コピー/貼り付け/リンクのコピー 等）

各ダイアログ（ConfirmDialog / RiskyConfirmDialog / ExportPickerDialog）は共通枠 `DialogShell.svelte`（オーバーレイ・ボックス・フォーカス制御・ボタン共通スタイル）を内包し、本文とキーボード処理のみを個別に持つ。状態は `PromiseDialog<T>`（`promiseDialog.svelte.ts`）を継承した各ストア。
```
...
- **読み込み**: 復元対象の各ファイルは起動時に**全件、内容まで読み込んで**タブを開く（`restoreSession`、`commands.ts`）。〔当初設計の「タブ選択時に内容を読み込む遅延読込」は未実装。起動が重くなる場合の将来最適化候補〕
- **不在ファイルの扱い**: 復元時にパスが存在しない場合、**ファイル1件ごとに通知ダイアログ**を表示し、ユーザーが選択する。
  - **再試行**: 再度パスの存在を確認（ファイルサーバーのマウント忘れ等を想定）。見つかれば開き、まだ無ければ再度ダイアログ。
  - **スキップ**: そのファイルの復元を諦める（タブを開かないため、次回保存されるセッションからは外れる）。
...
sequenceDiagram
  ...
  App->>App: 設定からファイル一覧を復元
  loop 各復元ファイル
    App->>Go: ファイル読込（ReadFile）
    alt 存在する
      Go-->>App: FileDoc（内容も即時読込）
    else 存在しない
      App->>U: 通知ダイアログ（再試行 / スキップ）
      alt 再試行
        U-->>App: 再確認（マウント後など）
      else スキップ
        U-->>App: 復元を中止（次回セッションから外れる）
      end
    end
  end
```
...
- **外部（リモート）画像**: 既定では**遮断**してレンダリングし、**ファイルごとに表示可否を確認**する。確認は危険な選択を伴うため、**§5.11 の共通ダイアログ（`RiskyConfirmDialog`）**を用いる（危険側「表示する」はポインタのみ＋起動遅延、安全側「表示しない」は Esc＋初期フォーカス）。結果は `Tab.remoteImagePolicy`（`allow` / `block`）として保持するが、**セッションには永続化しない**（次回起動時に再確認）。
...
- **About（README）**: 「Markmiru について...」（`menu:about`）→ `README.md`（概要・機能一覧等）を `//go:embed` で埋め込み、`ReadReadme()` で取得 → `openReadme()`。`ReadReadme()` は先頭に**バージョン（git ショート SHA）**を表示する（ldflags `-X main.version` で埋め込み。素のビルドでは `dev`）。OS のプロパティにも同じ版を埋め込む（AGENTS.md「バージョン番号」参照）。
...
- **閲覧モード**: コピー（Ctrl+C・選択時）／**リンクのコピー**（`href` の実リンク先。リンク以外で右クリックした場合は**無効**）／すべて選択（Ctrl+A）／検索…（Ctrl+F。§5.12）。**貼り付けは無し**。
- **編集モード（CodeMirror）**: 取り消し（Ctrl+Z）／再実行（Ctrl+Y）／切り取り（Ctrl+X）／コピー（Ctrl+C）／貼り付け（Ctrl+V）／すべて選択（Ctrl+A）／検索…（Ctrl+F）。取り消し/再実行は `undoDepth`/`redoDepth` で可否を判定。編集操作は `editorBridge` 経由で取得した `EditorView` の組み込みコマンド（`undo`/`redo`/`selectAll`/`openSearchPanel`）またはトランザクション（貼り付け）で行う。
...
外部アプリ起動・外部通信など**ユーザーにリスクのある選択**を伴う確認は、共通の `RiskyConfirmDialog.svelte`（状態は `riskyDialog.svelte.ts`）で行う。**Chromium のガイドライン**（Security Considerations for Browser UI）に倣った防御を共通化している。

- **危険側ボタン（accept）はポインタ操作のみ**。Enter / Space などキーボードでは発動させない（**keyjacking 対策**。`tabindex=-1` ＋ キー入力の抑止、フォーカスも当てない）。
- **表示直後 約 500ms は危険側を無効化**（`disabled`）し、以降クリック可能にする（**clickjacking 対策**＝起動遅延）。
- **安全側ボタン（reject）は Esc でも実行**でき、初期フォーカスはダイアログ枠に置く（ボタンを誤発動させない）。**Enter は無視**する。

利用箇所:

- **外部リンクを開く**（閲覧モード）: `http(s)` / `mailto` のクリックで、`BrowserOpenURL` で OS の既定アプリへ渡す**前に**確認（`links.ts`）。URL を `detail` として表示。許可時のみ開く。ページ内アンカー（`#...`）や未知スキームは確認対象外（前者は通常遷移、後者はブロック）。
- **外部画像の読み込み**（§5.8）: 外部画像を含むファイルでファイルごとに確認（`Preview.svelte` / 復元時は `commands.restoreSession`）。危険側「表示する」／安全側「表示しない」。

### 5.12 ページ内検索

両モードとも **Ctrl+F とコンテキストメニュー「検索…」**から起動できる（埋め込み WebView にはブラウザ標準のページ内検索 UI が無いため、いずれも自前で用意する）。

- **編集モード**: CodeMirror の検索パネルをそのまま使う（`openSearchPanel(view)`。Ctrl+F と同等）。
- **閲覧モード**: 自前の**フローティング検索バー**（`Preview.svelte` ＋ 状態 `viewFind.svelte.ts`）を本文右上に重ねて表示する（本文はずれない）。
  - 入力欄・一致件数「現在/総数」・前へ/次へ（Enter＝次、Shift+Enter＝前）・閉じる（Esc / ✕）。大文字小文字は区別しない。
  - 一致箇所は **CSS Custom Highlight API** で強調（DOM を書き換えない）。全一致は淡色、現在の一致は濃色。`scrollIntoView` で追従。
  - 本文の再描画時は Range が無効になるため、検索中なら作り直す。閲覧モードを抜ける（Preview のアンマウント）と検索を閉じてハイライトを解除する。
...
```text
Markmiru/
├─ wails.json                     # Wails 設定。info.productVersion(版)・postBuildHooks(darwin/* で macOS アイコンを .app へ上書き)
├─ go.mod
├─ scripts/                       # ビルドスクリプト（git ショート SHA を版として埋め込む）
│  ├─ build.ps1                   # Windows（動作確認済み）
│  └─ build.sh                    # macOS/Linux（macOS 検証済み / Linux 未検証）
├─ main.go                       # Wails 起動・ネイティブメニュー・ライフサイクル・version 変数
...
│  │  ├─ App.svelte
│  │  ├─ lib/
│  │  │  ├─ components/          # TabBar / OpenFilesList / Toolbar / Preview / Editor / SettingsPanel / ColorField / DialogShell / ConfirmDialog / ExportPickerDialog / RiskyConfirmDialog / ContextMenu
...
│  │  │  ├─ promiseDialog.svelte.ts # ダイアログ状態の共通基底（open＋Promise 解決）
│  │  │  ├─ dialog.svelte.ts     # 確認ダイアログの状態
│  │  │  ├─ stylePicker.svelte.ts # エクスポート対象選択ピッカーの状態
│  │  │  ├─ riskyDialog.svelte.ts # 危険な選択の確認ダイアログの状態（外部リンク/外部画像で共有）
│  │  │  ├─ viewFind.svelte.ts   # 閲覧モードのページ内検索バーの開閉状態
│  │  │  ├─ editorBridge.ts      # 編集モードの EditorView 受け渡し（右クリック/編集メニュー用）
│  │  │  ├─ editActions.ts       # 編集アクション（コピー/貼り付け/取消 等）。右クリックと非mac編集メニューで共有
│  │  │  ├─ dom.ts               # 汎用 DOM ユーティリティ（要素内容の全選択 等）
...
| 編集 | **macOS**: Wails `EditMenu()`（ネイティブ標準メニュー。元に戻す/コピー/貼り付け 等）。**Windows・Linux**: 日本語ラベルで手組み（取り消し/やり直し/切り取り/コピー/貼り付け/すべて選択 / 検索...）。各項目は `menu:*` を発火し、右クリックメニューと共有の `editActions.ts` で実行（クリップボードは Go ランタイム経由）。ショートカットは**併記のみ**で、編集専用項目は**閲覧モードで非活性**（後述） |
...
> - **ショートカットは「表示のみ」**: ラベルに `\t` ＋ キー表記を埋め込み Win32 に右寄せ表示させる。アクセラレータ（`AddText` の第2引数）には**登録しない**＝WebView/CodeMirror のネイティブなキー処理や、設定パネル・検索バー等の入力欄での Ctrl+C/V/X を奪わないため（キー操作は元々ネイティブで動作する。メニューはクリック呼び出し＋ショートカット周知用）。
> - **編集専用項目の非活性**: 取り消し/やり直し/切り取り/貼り付けは**閲覧モードでは非活性**にする。フロントの `App.svelte` がアクティブタブのモード変化時に `SetEditMenuEnabled(canEdit)`（バインド）を呼び、Go が該当 `*menu.MenuItem` の `Disabled` を切り替えて `runtime.MenuUpdateApplicationMenu` で再描画する（保持先は `main.go` の `editOnlyMenuItems`）。コピー/すべて選択/検索は両モードで有効。
> - **検索**: `menu:find` → `editActions.menuFind()`（編集モードは CodeMirror の検索パネル、閲覧モードはページ内検索バー `viewFind`）。
